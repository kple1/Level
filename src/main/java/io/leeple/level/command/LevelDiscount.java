...
public class LevelDiscount implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player player) {
            ...
            String expString = playerConfig.getString("EXP"); // "현재 경험치/최대 경험치" 형식의 문자열을 가져옴
            ...
            playerConfig.set("EXP", newExpString); // 새로운 문자열로 "EXP" 키의 값을 업데이트
            Main.getPlugin().saveYamlConfiguration(playerConfig, playerFile); // 업데이트된 설정 파일을 저장
